...
| `oidc.cookieSecret.autoGenerate` | bool | `true` | mint a cookie-secret on first boot if file is missing; set false to require explicit staging |
...
**`client-id` and `client-secret` are required** — they come from the IdP and the loader cannot invent them. Missing → unit fails loudly.

**`cookie-secret` is auto-generated on first run** by default (`cookieSecret.autoGenerate = true`). It's 32 bytes of local randomness used to sign / encrypt session cookies; it never leaves the cluster, so on-target generation is safe. The minted file persists across reboots — sessions remain valid until the file is replaced or deleted.

**Length is load-bearing:** OAuth2-Proxy uses the cookie-secret bytes directly as an AES key, so the value must be exactly 16, 24, or 32 bytes. The auto-gen recipe is `openssl rand -base64 32 | head -c 32` — the `| head -c 32` is **not optional**. `openssl rand -base64 32` alone outputs 44 chars + newline = 45 bytes, which OAuth2-Proxy rejects at startup (`cookie_secret must be 16, 24, or 32 bytes to create an AES cipher, but is 45 bytes`). The loader also validates pre-staged file lengths and fails clearly on the wrong size.

Disable auto-generation (`cookieSecret.autoGenerate = false`) when the cookie-secret is managed externally (agenix / sops-nix decrypting into a tmpfs path that must be predictable across reboots, or any time you want the loader to fail loudly on a missing cookie-secret rather than silently mint a new one).
...
# 4. File-level: are the IdP credentials staged?
ls -lah /var/lib/cert-manager/ /var/lib/oauth2-proxy/
# Expect: ca.{crt,key} for cert-manager;
# client-id + client-secret for oauth2-proxy (cookie-secret is
# auto-generated by the loader on first run by default).
# If autoGenerate was disabled or you need to force a rotation:
