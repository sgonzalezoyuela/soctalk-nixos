...
| Setting `cookieSecret.autoGenerate = false` without explicitly staging `cookie-secret` | Loader fails with "cookie-secret missing or empty"; Pod CrashLoopBackOff | Either flip `autoGenerate` back to true, or `openssl rand -base64 32 \| tr -d '\n' > /var/lib/oauth2-proxy/cookie-secret && systemctl restart oauth2-proxy-secrets.service` |
